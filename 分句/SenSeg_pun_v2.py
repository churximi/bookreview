...
时间：2016年4月14日 20:40:26
性能：目前跟哈工大语言云的分句基本一致（测试了3000句子，分句100%一致）2016年4月12日测试
改进：使用正则表达式re.compile搭配re.findall分句，取代原来的split
"""
...
def fenju(par, par2):                                # par——文件名；par2——文件编号
    ...
    # filepath = os.path.join(path, par)             # 文件路径
    filepath = path + "\\" + par
    ...
    for x in range(len(contents)):                           # 书评数量
        ...
        senlist = []
        sen_pat = re.compile(u'.*?[。！？；…]')
        allsen = re.findall(sen_pat, content)
        for sen in allsen:
            if len(sen) > 1:
                if u"…" in sen:
                    senlist.append(sen+u"…")
                else:
                    senlist.append(sen)
        ...
        for j in range(len(content)):                # 考虑到分割的最后一个为空，不是句子，所以-1
